        ...
        <div class="row">
            <div class="col-md-8">
                <div class="post-container">
                    ...
                    INNER JOIN user ON user.user_id = post.author WHERE post.title LIKE '%{$search}%' OR post.description LIKE '%{$search}%' ORDER BY post.post_id DESC LIMIT {$offset}, {$limit}";

                    $postQuery = mysqli_query($con, $postSql);
                    if (mysqli_num_rows($postQuery) > 0) {
                        while ($postData = mysqli_fetch_assoc($postQuery)) {
                    ?>
                            <h3><?php echo $postData['category_name']; ?></h3>
                            <br>
                    ...
                    <?php
                    $pagiSql = "SELECT * FROM post WHERE title LIKE '%{$search}%' OR description LIKE '%{$search}%'";
                    ...
                    ?>
                        <ul class='pagination admin-pagination'>
                            ...
                            <?php if ($page > 1) { ?>
                                <li><a href="<?php echo base_url; ?>/search.php?search=<?php echo $search; ?>&page=<?php echo $page - 1; ?>"> <i class="fa fa-long-arrow-left" aria-hidden="true"></i> </a></li>
                            <?php } ?>

                            <?php for ($i = 1; $i <= $total_pages; $i++) { ?>
                                <li><a class="<?php echo ($page == $i) ? 'active' : '' ?>" href="<?php echo base_url; ?>/search.php?search=<?php echo $search; ?>&page=<?php echo $i; ?>"> <?php echo $i; ?> </a></li>
                            <?php } ?>

                            <?php if ($total_pages > $page) { ?>
                                <li><a href="<?php echo base_url; ?>/search.php?search=<?php echo $search; ?>&page=<?php echo $page + 1; ?>"> <i class="fa fa-long-arrow-right" aria-hidden="true"></i> </a></li>
                            <?php } ?>
                        </ul>
                    <?php } ?>
                </div><!-- /post-container -->
